...
- **Hardware & Session Permissions Management (Universal Deny-by-Default with Reactive Sync):** Controls hardware capability delegation across the default session and all partitioned guest sessions (`session.setPermissionRequestHandler` and `session.setPermissionCheckHandler`) under a strict **Deny-by-Default** security posture, permitting only explicitly user-granted permissions (`userData/permissions.json`) for Microphone, Camera, Location, Screen Sharing, and Notifications (plus clipboard access for media pasting), while proactively configuring all existing and newly created account partitions (`persist:acc_*`) and broadcasting updates reactively across all active WebContents in real time.
...
- **Bifurcated Offline Protections, DinoGame Module & Dynamic Offline Duration Counter:**
  - Scoped strictly to `#webview-container` (`z-index: 20`), guaranteeing that the lateral sidebar (`<aside class="sidebar">`) and full-window native views (`#settings-view`, `#donations-view`, `#doom-view`) remain 100% accessible and functional even without Internet connectivity. Automatically hides guest `<webview>` elements (`visibility: hidden`) while active to prevent Linux rendering occlusion.
  - **Escenario A (Pérdida en caliente durante el uso):** Passive wait state. Displays "Sin conexión a internet" with a non-clickable status badge ("Buscando señal...") and top radar spinner. Strictly hides the "Volver a escanear" button. Upon reconnection via active Node.js background polling (every 6 seconds), silently dismisses the overlay and destroys minigame instances without reloading the `<webview>` instances, preserving user drafts and open chat states.
  - **Escenario B (Arranque offline / En frío):** Active wait state enforced imperatively at cold launch by `checkInitialNetworkState()`, bypassing disk-cached partition events. Displays "Sin conexión a internet" with subtitle "No se ha encontrado conexión a internet, por favor vuelva a escanear..." and an interactive button "Volver a escanear". Strictly hides the "Buscando señal..." badge. On click, immediately disables itself (`pointer-events: none`, `opacity: 0.7`) and renders an internal spinner while validating connectivity against `https://web.whatsapp.com` (HTTP 200–299); on failure, reactivates in idle state, and if restored, reloads accounts, cleans timers/minigame, and resumes normal service.
  - **Modular T-Rex Runner (`src/renderer/dino-game.js`):** Independent, clean OOP minigame module loaded lazily on demand. Features 300px vertical canvas height (`groundY = 260px`), dynamic theme color token extraction, 3-tier aerial Pterodactyls (low, mid, high altitudes), strict keyboard controls (`ArrowUp` to jump, `ArrowDown` to duck with 24px reduced hitbox and crawl sprite), audio beeps, score persistence, and a strict `destruir()` lifecycle hook that cancels animation frames and removes window listeners to eliminate memory/CPU leaks upon reconnection.
  - **Dynamic Offline Duration Timer:** Centered below the minigame canvas, reactively formats elapsed downtime every second (`Estamos sin conexión desde hace {tiempo} {medición}.`) across seconds, minutes, hours, and days, resolving strictly through all 55 i18n locale dictionaries without hardcoding.
