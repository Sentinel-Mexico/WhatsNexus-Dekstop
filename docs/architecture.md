...
- **Bifurcated Offline Protections, DinoGame Module & Dynamic Offline Duration Counter:**
  ...
  - **Escenario B (Arranque offline / En frío):** Active wait state. Displays "Sin conexión a internet" with subtitle "No se ha encontrado conexión a internet, por favor vuelva a escanear..." and an interactive button "Volver a escanear". Strictly hides the "Buscando señal..." badge. On click, triggers a background connectivity check with spinner state; if unreachable, gracefully resets to idle, and if restored, reloads accounts and resumes normal service.
